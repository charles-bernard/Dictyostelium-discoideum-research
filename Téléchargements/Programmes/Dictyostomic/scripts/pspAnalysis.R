...
references = read.table("output_files/pspGenes/pspGenesInfo.txt", skip = 1, header = T, sep = "\t");
geneNames = references[,2];
row_lifeCycle = as.integer(references[,3]);
row_phenotype = as.integer(references[,4]);
n = length(geneNames)

parikh_phenotype = read.table("input_files/parikh_normalized_data.txt")
...
cellType_differential = mean_psp / mean_pst
...
pdf("output_files/pspGenes/expressionClusters.pdf", height = n + 15 * (1/log10(n)), width = 30)
...
title("Hierarchy of Ratios:\n Expression in Psp cells / Expression in Pst cells", outer = T, line = -12, cex.main = 5)
