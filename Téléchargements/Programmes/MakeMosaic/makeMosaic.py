...
def askConfig(mosaicSize, channels):
	...
	return channel, int(imgReductionFactor), isVideo, int(videoReductionFactor)
...
def convert_uint16Grayscale_to_RGB(uint16Grayscale):
...
def makeMosaic(positionFolders, channel, date, channelImgNames, imgSize, mosaicSize, rowEnd, colEnd, tEnd, tSpan, imgReductionFactor, isVideo, videoReductionFactor):
	...
	for t in range(0, nb_t):
		...
		mosaicPath = outputDirectory + '/t_' + '%4d' % (t*tSpan) + '_' + channel + '.tif'
		...
		if isVideo == 'y':
			if videoReductionFactor == 1:
				...
				videoMosaic = convert_uint16Grayscale_to_RGB(reducedMosaic)
			else:
				videoMosaic = cv2.resize(mosaic, (videoSize[1], videoSize[0]), interpolation=cv2.INTER_LANCZOS4)
				videoMosaic = convert_uint16Grayscale_to_RGB(videoMosaic)
...
def main():
	...
	channel, imgReductionFactor, isVideo, videoReductionFactor = askConfig(mosaicSize, channels)
	...
	makeMosaic(positionFolders, channel, date, channelImgNames, imgSize, mosaicSize, rowEnd, colEnd, tEnd, tSpan, imgReductionFactor, isVideo, videoReductionFactor)
