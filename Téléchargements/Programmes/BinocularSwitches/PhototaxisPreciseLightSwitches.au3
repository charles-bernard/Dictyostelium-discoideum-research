...
Func Main()
   ...
   ;So far, nbPositions must be <= 3 since there are only 3 white leds plugged in the Arduino
   Local $switchPosition = 0
   ;The switchPosition determines which white led is currently turned on

   Local const $dark = False
   ;Boolean to test wheither or not the slugs keep the memory of their direction
   ;when the light is turned off

   Local const $pictureTime_normal = 10
   ;The pictureTime (in min) determines the intervall of time that separates two pictures
   Local const $pictureTime_afterSwitch = 2
   Local const $nbPictures_afterSwitch = 5
   Local $pictureCounter = 0
   Local $pictureTime = $pictureTime_normal

   Local const $lightTime = 180
   ...
   ;The Program runs until the user exits it manually
	  ...
	  Else
		 If mod($currentMin, $switchTime) = 0 Then
			If $switchPosition = 0 Then
			   ...
			   Else
				  ...
				  $pictureTime = $pictureTime_afterSwitch
				  $pictureCounter = 1
			   EndIf
		 ...
		 If mod($currentMin, $pictureTime) = 0 Then
			...
			If $pictureCounter > 5 Then
		 		$pictureCounter = 0
		 		$pictureTime = $pictureTime_normal
		 	ElseIf $pictureCounter > 0 Then
		 		$pictureCounter += 1
		 	EndIf
